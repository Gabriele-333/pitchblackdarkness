...
public final class PbdConfigScreen extends Screen {
    ...
    @Override
    protected void init() {
        var platform = PbdLevels.platform();
        ...
        addRenderableWidget(toggle("moonMatters", x, y, platform.moonMatters(),
                (button, value) -> apply(() -> PbdLevels.platform().setMoonMatters(value))));
        y += ROW_HEIGHT;

        addRenderableWidget(toggle("affectNether", x, y, platform.affectNether(),
                (button, value) -> apply(() -> PbdLevels.platform().setAffectNether(value))));
        y += ROW_HEIGHT;

        addRenderableWidget(toggle("affectEnd", x, y, platform.affectEnd(),
                (button, value) -> apply(() -> PbdLevels.platform().setAffectEnd(value))));
